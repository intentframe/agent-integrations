...
| `actions.manifest` (repo) | **Dev** | Static list of all `mapper: generic` action IDs (full catalog superset) |
| `actions.manifest` (runtime) | **Copied once** | Seeded on `integrate hermes`; never overwritten by automation |
...
2. Regenerate committed `actions.manifest` to include the new action ID
