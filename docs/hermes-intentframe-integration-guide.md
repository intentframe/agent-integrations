...
**Uninstall / reinstall:** `intentframe-integrations uninstall hermes` removes IntentFrame; add `--remove-hermes` to wipe `~/.hermes` too. After uninstall, re-run the [install script](../scripts/install-hermes-plugin.sh) — not `integrate`. Details: [hermes-cli.md#uninstall](hermes-cli.md#uninstall). Caveats (root FHS, version pins): [hermes-known-limitations.md](hermes-known-limitations.md).
